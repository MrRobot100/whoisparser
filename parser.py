...
class WhoisEntry(dict):
    ...
    _regex = {
        ...
        'registrant_org':                    'Registrant\s*Organization: *(.+)',
...
class WhoisUs(WhoisEntry):
    ...
    regex = {
        ...
        'registrant_name':                'Registrant Name: *(.+)',
        'registrant_address':             'Registrant Address1: *(.+)',
        ...
        'admin_postal_code':              'Admin Postal Code: *(.+)',
        'admin_country':                  'Admin Country: *(.+)',
        'admin_country_code':             'Admin Country Code: *(.+)',
        'admin_phone':                    'Admin Phone : *(.+)',
        'admin_email':                    'Admin Email: *(.+)',
        ...
        'tech_address':                   'Technical Street: *(.+)',
        'tech_address2':                  'Technical Contact Address2: *(.+)',
        'tech_city':                      'Technical City: *(.+)',
        'tech_state':                     'Technical State/Province: *(.+)',
        'tech_postal_code':               'Technical Postal Code: *(.+)',
        'tech_country':                   'Technical Country: *(.+)',
        'tech_country_code':              'Technical Country Code: *(.+)',
        'tech_phone':                     'Technical Phone: *(.+)',
        'tech_email':                     'Technical Email: *(.+)',
...
class WhoisPl(WhoisEntry):
    ...
    regex = {
        ...
        'name_servers':                   '(?<=nameservers:).*[\s\S]*?(?=created)',
    }
    ...
    def __init__(self, domain, text):
        if 'No information available about domain name' in text:
            ...
            WhoisEntry.__init__(self, domain, text, self.regex)
            try:
                self['name_servers']=self['name_servers'].replace('\n','').replace('   ','').split('\r')
            except:
...
class WhoisCa(WhoisEntry):
    ...
    def __init__(self, domain, text):
        if 'Domain status:         available' in text:
            ...
            WhoisEntry.__init__(self, domain, text, self.regex)
            try:
                self['registrar']=re.sub(' +','', self['registrar']).replace('Name:','')
            except:
                self['registrar']=None
            try:
                self['name_servers']=re.sub(' +','',self['name_servers']).split('\n')
            except:pass
...
class WhoisEu(WhoisEntry):
    ...
    regex = {
        ...
        'name_servers':'(?<=Name servers:).*[\s\S]*?(?=Please)',
    }
...
class WhoisBg(WhoisEntry):
    ...
    regex = {
        ...
        'name_servers':'(?<=NAME SERVER INFORMATION:).*[\s\S]*?(?=\t)',
...
class WhoisBe(WhoisEntry):
    ...
    regex = {
        ...
        'fax': 'Fax: *(.+)',
        'emails': 'Email: *(.+)',
        'name_servers': '(?<=Nameservers:).*[\s\S]*?(?=Keys)'
    }
    ...
    def __init__(self, domain, text):
        if 'Status: AVAILABLE' in text:
            ...
            WhoisEntry.__init__(self, domain, text, self.regex)
        try:
            self['name_servers']=re.sub(' +','',self['name_servers'])
            self['name_servers']=re.sub('\t','',self['name_servers'])
            self['name_servers']=re.sub('\r','',self['name_servers']).split('\n')
        except:pass
...
class WhoisDk(WhoisEntry):
    ...
    def __init__(self, domain, text):
        if 'No match for ' in text:
            ...
            WhoisEntry.__init__(self, domain, text, self.regex)
        try:
            self['name_servers']=re.sub('Hostname:','',self['name_servers'])
            self['name_servers']=re.sub(' +','',self['name_servers']).split('\n')
        except:pass
...
class WhoisIn(WhoisEntry):
    ...
    regex = {
        ...
        'admin_phone':        'Admin Phone:*(.+)',
        'admin_email':        'Admin Email:*(.+)',
        'tech_address':       'Admin Street1:*(.+)',
        'tech_name':          'Admin Name:*(.+)',
        'tech_city':          'Admin City:*(.+)',
        'tech_state':         'Admin State/Province:*(.+)',
        'tech_postal_code':   'Admin Postal Code:*(.+)',
        'tech_country':       'Admin Country:*(.+)',
        'tech_phone':         'Admin Phone:*(.+)',
        'tech_email':         'Admin Email:*(.+)',
